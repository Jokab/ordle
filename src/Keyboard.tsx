...
export default function({guesses, letterClick, enterClick, backspaceClick}: KeyboardProps) {
    ...
    const drawLetters = (keys: Letter[], startIndex: number, endIndex: number) => {
      ...
      for (let i = startIndex; i < endIndex; i++) {
        const cellClass = (letter: Letter) => classNames({
          'text-xl text-center truncate font-bold w-8 rounded-md': true,
    ...
    return (
      <div className="flex items-center justify-center flex-col text-sm">
        ...
        <div className="flex mt-2 items-center justify-between gap-1">
          <div className="bg-gray-400 rounded-md font-bold text-2xl  p-1" onClick={enterClick}>Enter</div>
          <div className="flex items-center justify-between gap-1">
            {drawLetters(usedKeys, 22, 29)}
          </div>
          <div className="bg-gray-400 rounded-md font-bold text-2xl p-1" onClick={backspaceClick}>Radera</div>
        </div>
      </div>
    )
  }